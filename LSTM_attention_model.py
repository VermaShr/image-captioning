...
class LSTM_attention_Model(object):
    """
    - DOES: defines the LSTM_attention model.
    """
    ...
    def add_captions_input(self):
        ...
        with tf.variable_scope("LSTM_att_captions_embed"):
            ...
            self.captions_input = tf.nn.embedding_lookup(word_embeddings,
                        self.captions_ph)
            print("printing captions input size")
            print(self.captions_input.shape)
    ...
    def add_logits(self):
        ...
        state), feeds each input through an LSTM, producing a hidden state vector
        ...
        initial_state = stacked_LSTM_cell.zero_state(tf.shape(self.captions_input)[0],
                    tf.float32)
        ...
        with tf.variable_scope("LSTM_attention"):
            ...
            # stacked LSTM cell, get the (top) hidden state vector and the
            ...
            for timestep in range(self.config.max_caption_length):
                if timestep > 0:
                    # make sure we reuse variables in the stacked LSTM cell:
                ...
                if timestep == 0:
                    ...
                    a = tf.split(a, self.config.no_of_img_feature_vecs, 0)
                    ####################################
                    ...
                    #print(self.captions_input[0].shape)
                    #print(self.config.no_of_img_feature_vecs.shape)
                ...
                # the timestep:
                print("step_captions_input with time step")
                print(self.captions_input.shape)
                print("timestep")
                print(timestep)
                ...
                print(step_captions_input.shape)
                
                # concatenate the timestep's img input and caption input to
                # get the network input:
                print("step_imgs_input shape")
                print(step_imgs_input.shape)
                print("step captions input shape")
                print(step_captions_input.shape)
                step_input = tf.concat([step_imgs_input, step_captions_input], 1)
                # (step_input has shape [batch_size, 600])

                # get the (top) hidden state vector for the timestep:
                output, new_state = stacked_LSTM_cell(step_input, previous_state)
...
def main():
    ...
    # create an LSTM_attention model object:
